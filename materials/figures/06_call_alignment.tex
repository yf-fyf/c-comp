% コマ6: call 前の16バイトアラインメント — 一時値が奇数個のときだけ sp が8ずれる
...
\begin{tikzpicture}[node distance=0mm,
  ...
  \node[padcell] (pad3) at (92mm, -12mm) {調整の 8バイト};
  \node[spmark] (sp3) at (75mm, -16mm) {sp};
  \draw[regptr] (sp3.east) -- (77mm, -16mm);
  \node[state] at (92mm, -19mm) {{\ttfamily depth = 1}\\調整して境界の上へ};
  ...
  \node[freecell] (pad4) at (138mm, -12mm) {調整分は返した};
  ...
  \node[gannot, anchor=north, align=center] at (69mm, -52mm)
      {引数の push は {\ttfamily call} の時点では戻し済みなので、ずれを決めるのは
       ...
       調整を入れるのは {\ttfamily call} の前後だけで、同じ幅を戻すため
       積んである一時値の位置は変わらない};
\end{tikzpicture}
\end{document}
