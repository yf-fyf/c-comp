...
コマ8 では `&` と `*` を実装し、ポインタも整数もすべて8バイト値として扱った。
...
この言語に配列はない。連続した領域を扱う話（`malloc` と添字）はコマ10 で扱う。
...
コマ8 で `codegen_lval` と `codegen` を分けたのと同じ理由である。
...
コマ8では常に `ld` / `sd` でよかったが、この回からは型サイズを見て命令を選ぶ。
...
```bash
python3 scaffold/test_runner.py sessions/09_types
```
...
```bash
python3 sessions/09_types/mycc.py sessions/09_types/tests/load_store_ty.c \
...
添字 `p[i]`・ポインタ演算・`sizeof(型名)`・前置 `++` の型対応はコマ10 で扱う。
この回の段階では、これらを含むプログラムはコンパイルできない。

この回の `size_of_ty_str(ty)` は引数が1つでよい。
構造体サイズを引くために `self._struct_defs` を渡す2引数版になるのはコマ13 からである。
