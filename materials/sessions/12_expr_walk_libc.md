...
コマ11 で、文字列を集める走査の仕組み（`_intern` / `emit_data_section` / `codegen_Str`）は
すでに全部そろっている。足りないのは走査の**網羅**だけである。
コマ11 の `collect_strings_expr()` は `Str` / `Assign` / `Call` の3種類しか枝を持っておらず、
それ以外の式の下にある文字列は素通りしていた。

```c
if ("abc"[1] != 'b') {   // Index の下にある "abc" を、コマ11 はまだ拾えない
    return 1;
}
```

この回でやるのは、コマ11 で文に対して書いたのと同じ形のハンドラを、式の種類の数だけ書くことである。
...
コマ11 版を上書きし、すべての式の種類を枝に持つ）。実装するのは呼ばれる側だけである。
...
`importlib` でコマ11 の `Codegen11` を継承した `Codegen12` に、以下を追加する。
...
構造体そのものを扱えるようになる**コマ13 を待たずにこの回で使える**。

コード生成としては、コマ7 の関数呼び出しとコマ9 の `ty_str` 管理がそのまま働くので、
...
```bash
python3 scaffold/test_runner.py sessions/12_expr_walk_libc
```

コマ11 のテストも引き続き通ることを確かめる。
`Codegen12` はコマ11 の実装を継承しているので、両方が通って初めて走査が完成したと言える。

```bash
python3 scaffold/test_runner.py sessions/11_strings_data_section
```

個別に動かす場合は、次のようにする。

```bash
python3 sessions/12_expr_walk_libc/mycc.py sessions/12_expr_walk_libc/tests/str_in_expr.c \
