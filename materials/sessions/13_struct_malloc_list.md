...
統合の圧縮方針（T118 で決定・着手時の記録）
  ...
  出典: 統合前の旧コマ12「struct」の原稿（414行）と旧コマ13「sizeof + malloc + 連結リスト」の原稿（412行）。
...
# コマ13: 構造体とヒープ（struct / `.` / `->` / `sizeof` / `malloc` / 連結リスト）
...
コマ10までに、型サイズとポインタ演算を扱えるようになった。この回では、複数のフィールドを
...
| `sizeof(struct タグ)` | `sizeof(struct Node)` | 構造体のサイズを引ける（`sizeof(int)` 等はコマ10） |
| `malloc` で構造体を確保 | `malloc(sizeof(struct Node))` | 確保先を構造体として扱う（`malloc` 呼び出し自体はコマ10） |
...
```bash
python3 scaffold/parse_viewer.py sessions/13_struct_malloc_list/tests/dot_access.c
```
...
```bash
python3 scaffold/parse_viewer.py sessions/13_struct_malloc_list/tests/arrow_access.c
```
...
`sizeof` の AST はコマ10 で見たものと同じである。

```bash
python3 scaffold/parse_viewer.py sessions/13_struct_malloc_list/tests/sizeof_test.c
```
...
```bash
python3 scaffold/parse_viewer.py sessions/13_struct_malloc_list/tests/list_min.c
```
...
- `malloc(...)` はただの `call` である。関数呼び出しはコマ7 で作ったものがそのまま働く。
...
コマ9 の `size_of_ty_str(ty)` は `int` / `char` / ポインタしか知らなかった。
`struct Point` のサイズは定義を見ないと分からないので、この回で第2引数が増える。

```python
size_of_ty_str(ty_str) -> int                    # コマ9〜コマ12
size_of_ty_str(ty_str, struct_defs=None) -> int  # この回以降
```

`struct_defs` は省略でき、省略した場合の結果はコマ9 と同じである。
そのため、コマ12 までに書いた呼び出しはそのまま動く。
...
この引数を渡す形に直す必要がある。以降のコマ14〜コマ16 でもこの2引数の形を使う。
...
## `sizeof` のコード生成 — コマ10 からの差分

`sizeof(型名)` そのものはコマ10 で導入済みである。
...
| 式 | サイズの求め方 | 導入回 |
|----|----------------|--------|
| `sizeof(int)`・`sizeof(char)`・`sizeof(int *)` | 型ごとに決まった値 | コマ10 |
...
`#include "lib.h"`（コマ11）でこの定義が取り込まれ、
...
`importlib` でコマ12 の `Codegen12` を継承した `Codegen13` に、以下の機能を追加する。
...
2〜5 の4つは、コマ12 まで動いていたコードに `self._struct_defs` を足すだけの修正である。
ただし直し忘れに気づけるよう、スケルトンでは4つとも `raise NotImplementedError` を置き、
コマ12 版のコードは TODO コメントの中に残してある。

11・12 の2つを書けば `malloc` と自己参照構造体と連結リストが動く。
`malloc` 呼び出しはコマ7 の関数呼び出しが、`->` は 1〜10 で書いた構造体処理が、
そのまま働くからである。
...
14. `malloc_struct.c` / `list_min.c` を通す<br>（`parse_struct_defs()` は自己参照フィールド `struct Node *next` をそのまま扱える。ポインタは中身を知らなくてもサイズ 8 で確定するからである。`malloc(sizeof(struct Node))` はコマ7 の関数呼び出しがそのまま働くので、追加の実装は要らない。）
15. `list_sum.c` まで通す
...
```bash
python3 scaffold/test_runner.py sessions/13_struct_malloc_list
```
...
```bash
DIR=sessions/13_struct_malloc_list
...
生成したアセンブリでメンバの offset 計算や `malloc` が返すアドレスを1命令ずつ確認したいときは、[RV64 シミュレータ](../../tools/sim.html) に貼り付ける。
