...
- **優先順位の低い演算子ほど、根に近い側に置かれる。**
  `1 + 2 * 3` の根が `add` で `mul` がその子になるのは、`+` が `*` より優先順位が低いからである。
- **同じ優先順位の演算子が並んだときは、左から順にまとめる（左結合）。**
  `10 - 3 - 2` は `(sub (sub (num 10) (num 3)) (num 2))` となり、`(10 - 3) - 2` を表す。
- **カッコは、この規則に優先して木の形を直接指定する。**
  `(1 + 2) * 3` では逆に `add` が `mul` の子になる。

この3つの規則で、どの式についても届く木の形が決まる。演算子ごとの優先順位は、
後述の言語仕様の節で**優先順位表**として示す。疑わしいときは `parse_viewer.py` に
...
1. スケルトンの `eval_ast` に `case 'Num'` が実装例として書かれていることを確認する（`node.val` を返すだけ）
2. `case 'Add'` / `case 'Sub'` / `case 'Mul'` を埋める（`eval_ast(node.lhs)` と `eval_ast(node.rhs)` を再帰で求め、足す・引く・掛ける）
3. `case 'Neg'` を埋める（`eval_ast(node.operand)` の符号を反転する）
4. `case 'Div'` / `case 'Mod'` を埋める（提供済みの `c_div` / `c_mod` に左右の評価結果を渡す。Python の `//` と `%` は使わない）

`case _` で `RuntimeError` を上げる分岐と、`main` の `return` 式を `eval_ast` へ渡す
`run_main(prog)` は、あらかじめスケルトンに書かれている。
...
| ファイル | 内容 | 通る目安 | 期待値 |
|----------|------|----------|--------|
| `add_mul.c` | `1 + 2 * 3` | 手順2 | 7 |
| `div_mod.c` | `100 / 4 + 17 % 5` | 手順4 | 27 |
| `paren.c` | `(10 - 3) * 2` | 手順2 | 14 |
| `neg.c` | `-3 + 10` | 手順3 | 7 |
| `all_ops.c` | `1 + 2 * 3 - 8 / 2` | 手順4 | 3 |
| `expression.c` | `1 + 2 * 3` | 手順2 | 7 |
...
```bash
python3 sessions/01_interpreter/mycc.py sessions/01_interpreter/tests/add_mul.c
```
