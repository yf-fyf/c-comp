...
[`language_spec.md` の「字句トークン」](../../workbook/docs/language_spec.md#grammar)にある。
...
```bash
python3 scaffold/parse_viewer.py sessions/10_strings_data_section/tests/printf_number.c
```
...
![`printf_number.c` の AST](figures/ast/10_printf_number_ast.svg)
...
![`.data` に置いた文字列と、`.text` からの参照](figures/10_sections.svg)
...
1. スケルトンの `importlib` 継承によりコマ9の Codegen クラスを引き継ぐ（あらかじめ書かれている）
...
5. `.text` セクションの出力と関数本体の生成は、コマ9までの実装がそのまま使われる（この回で新しく書く部分はない）
