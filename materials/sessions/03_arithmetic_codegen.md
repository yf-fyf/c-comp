...
コマ2では、AST を直接評価して Python の整数値を返した。
...
コマ2と同じように、まず Parser が返す AST を確認する。

```bash
python3 scaffold/parse_viewer.py sessions/03_arithmetic_codegen/tests/complex.c
```
...
コマ2の `eval_ast()` は、計算結果を Python の値として返していた。

```text
eval_ast(node) -> 整数値
```

コマ3の `codegen()` は、計算結果を直接返さない。
...
木のたどり方はコマ2と同じで、「値を返す」部分が「命令を出力する」に置き換わる。
...
この退避のパターンは、この後の代入（コマ4）や関数呼び出し（コマ7）でも繰り返し使う。
...
ここで埋めた TODO は、コマ4 以降に `importlib` の継承でそのまま引き継がれる。
...
```bash
python3 sessions/03_arithmetic_codegen/mycc.py sessions/03_arithmetic_codegen/tests/target.c \
...
生成したアセンブリを1命令ずつ実行して確認したいときは、[RV64 シミュレータ](../../tools/sim.html) に貼り付ける。
