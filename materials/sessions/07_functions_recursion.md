...
```bash
python3 scaffold/parse_viewer.py sessions/07_functions_recursion/tests/target.c
```
...
```bash
python3 scaffold/parse_viewer.py sessions/07_functions_recursion/tests/mutual_rec.c
```
...
コマ7では、関数プロトタイプに対してアセンブリを出す必要はない。
...
2. return用の共通ラベルを作る
3. _alloc_params() で funcdef.params を self.alloc_local() する  ← コマ7で追加
4. self.collect_decls(funcdef.body) でローカル変数を集める
5. frame_size = align_to(self._stack_offset, 16) を計算する
6. プロローグを出す
7. a0, a1, ... を params のスロットへ sd する  ← コマ7で追加
...
`importlib` でコマ6 の `Codegen06` を継承した `Codegen07` に、以下の機能を追加する（スケルトンにあらかじめ書かれている）。
...
```bash
python3 scaffold/test_runner.py sessions/07_functions_recursion
```
...
```bash
python3 sessions/07_functions_recursion/mycc.py sessions/07_functions_recursion/tests/target.c \
