...
コマ11a で、文字列を集める走査の仕組み（`_intern` / `emit_data_section` / `codegen_Str`）は
すでに全部そろっている。足りないのは走査の**網羅**だけである。
コマ11a の `collect_strings_expr()` は `Str` / `Assign` / `Call` の3種類しか枝を持っておらず、
それ以外の式の下にある文字列は素通りしていた。

```c
if ("abc"[1] != 'b') {   // Index の下にある "abc" を、コマ11a はまだ拾えない
    return 1;
}
```

この回でやるのは、コマ11a で文に対して書いたのと同じ形のハンドラを、式の種類の数だけ書くことである。
...
dispatch の `match` はスケルトンに書いてある（`Codegen11b.collect_strings_expr()` が
コマ11a 版を上書きし、すべての式の種類を枝に持つ）。実装するのは呼ばれる側だけである。
...
`importlib` でコマ11a の `Codegen11a` を継承した `Codegen11b` に、以下を追加する。
...
5 まで終われば、文字列リテラルはプログラムのどこに書かれていても `.data` に出るようになる。
7 で新しく書くコードは無い（理由は次節）ので、実装の山は 3 で越える。
...
コード生成としては、コマ8 の関数呼び出しとコマ10 の `ty_str` 管理がそのまま働くので、
...
コマ11a のテストも引き続き通ることを確かめる。
`Codegen11b` はコマ11a の実装を継承しているので、両方が通って初めて走査が完成したと言える。
...
```bash
python3 sessions/11b_expr_walk_libc/mycc.py sessions/11b_expr_walk_libc/tests/str_in_expr.c \
