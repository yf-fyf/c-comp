...
統合の圧縮方針（T118 で決定・着手時の記録）
  ...
  出典: materials/sessions/12_struct.md（414行）と 13_sizeof_malloc_list.md（412行）。
...
# コマ12+13: 構造体とヒープ（struct / `.` / `->` / `sizeof` / `malloc` / 連結リスト）
...
```bash
python3 scaffold/parse_viewer.py sessions/12plus13_struct_malloc_list/tests/dot_access.c
```
...
```bash
python3 scaffold/parse_viewer.py sessions/12plus13_struct_malloc_list/tests/arrow_access.c
```
...
```bash
python3 scaffold/parse_viewer.py sessions/12plus13_struct_malloc_list/tests/sizeof_test.c
```
...
```bash
python3 scaffold/parse_viewer.py sessions/12plus13_struct_malloc_list/tests/list_min.c
```
...
- `malloc(...)` はただの `call` である。関数呼び出しはコマ8 で作ったものがそのまま働く。
...
```python
size_of_ty_str(ty_str) -> int                    # コマ10〜コマ11
...
`importlib` でコマ11 の `Codegen11` を継承した `Codegen1213` に、以下の機能を追加する。
...
`malloc` 呼び出しはコマ8 の関数呼び出しが、`->` は 1〜10 で書いた構造体処理が、
そのまま働くからである。
...
14. `malloc_struct.c` / `list_min.c` を通す<br>（`parse_struct_defs()` は自己参照フィールド `struct Node *next` をそのまま扱える。ポインタは中身を知らなくてもサイズ 8 で確定するからである。`malloc(sizeof(struct Node))` はコマ8 の関数呼び出しがそのまま働くので、追加の実装は要らない。）
15. `list_sum.c` まで通す
...
```bash
python3 scaffold/test_runner.py sessions/12plus13_struct_malloc_list
```
...
```bash
DIR=sessions/12plus13_struct_malloc_list
