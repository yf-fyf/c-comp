...
![前処理の位置と、`BASE` が置換される場所・されない場所](figures/14_preprocess_pipeline.svg)
...
![`define_min.c` の AST](figures/ast/14_define_min_ast.svg)
...
![実際の C コンパイラの分割コンパイルと、今回の一括方式](figures/14_pipeline.svg)
