...
成果物はここで完成する。次は[コマ16（発表・振り返り）](16_demo_review.md)である。

コマ16 では新しい実装をしない。この回で完成させた `final/mycc.py` の設計を発表し、
...
長期チャレンジである。選び方の目安はコマ16で扱う。

上の完成チェックリストは、そのままコマ16 の発表の準備にあたる。
とくに「生成アセンブリを1つ以上読んで説明できる」「苦労したバグとその直し方を説明できる」の2つは、
コマ16 で実際に人に向けて話す部分なので、ここで手を抜かないこと。
