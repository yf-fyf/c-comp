...
第08回までの `codegen()` は、式を評価して値を `a0` に残す関数だった。
...
第08回で関数呼び出しを実装しているため、ポインタを引数として渡す仕組み自体は新しくない。
...
`importlib` でコマ08 の `Codegen08` を継承した `Codegen09` に、以下の機能を追加する（スケルトンにあらかじめ書かれている）。
