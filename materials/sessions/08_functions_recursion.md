...
![関数定義の AST](figures/ast/08_fib_ast.pdf)
...
![相互再帰の AST](figures/ast/08_mutual_rec_ast.pdf)
...
これは第07回のプロローグ・エピローグがすでに行っている。

![`fib(3)` を呼び出したときのスタックフレームの積み重なり](figures/08_call_frames.pdf)
...
![3引数の呼び出しで、push した引数をレジスタへ戻す対応](figures/08_arg_stack.pdf)
...
これは第03回から使っている二項演算の退避パターンで解決する。
...
第08回では、関数プロトタイプに対してアセンブリを出す必要はない。
...
これらは重要な話題だが、第08回ではまず、8個以下の整数引数を持つユーザー定義関数を呼び出せることを目標にする。

## 編集するファイル

- `mycc.py`

`importlib` でコマ07 の `Codegen07` を継承した `Codegen08` に、以下の機能を追加する（スケルトンにあらかじめ書かれている）。
...
```bash
python3 sessions/08_functions_recursion/mycc.py sessions/08_functions_recursion/tests/target.c \
...
`main` 以外の関数の `return` も、第05回と同じく共通エピローグラベルへジャンプする。
