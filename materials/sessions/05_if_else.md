...
```bash
python3 scaffold/parse_viewer.py sessions/05_if_else/tests/target.c
```
...
コマ4までは、`return` 文が関数の末尾にしか現れなかったため、`return` の後に直接エピローグを置けばよかった。

しかし、コマ5からは `if` の中などに `return` が現れる。
...
| `'Break'` | `gen_stmt_Break` | コマ6 で対応。コマ5 では未対応でよい |
...
`importlib` でコマ4 の `Codegen04` を継承した `Codegen05` に、以下の機能を追加する。
...
```bash
python3 scaffold/test_runner.py sessions/05_if_else
```
...
```bash
python3 sessions/05_if_else/mycc.py sessions/05_if_else/tests/target.c \
...
関数先頭で宣言した `y` そのものである。
宣言の収集はコマ4 の `collect_decls()` が関数本体の先頭を見るだけで済み、
...
条件は2つの `if` に分けて書くこと。
`&&` / `||` / `!` のコード生成を実装するのはコマ14 で、この回では扱わない。
短絡評価の実装は発展課題 S1 で扱う。

:::

生成したアセンブリの分岐が期待どおりに飛ぶか確認したいときは、[RV64 シミュレータ](../../tools/sim.html) に貼り付ける。
