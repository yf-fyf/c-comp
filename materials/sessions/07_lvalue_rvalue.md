...
ただしコマ3 の `codegen_lval()` が扱えるのは変数 `'Var'` だけで、
...
```bash
python3 scaffold/parse_viewer.py sessions/07_lvalue_rvalue/tests/deref_write.c
```
...
（コマ6 までは、返り値以外に呼び出し元へ結果を伝える手段がなかった）。
そのためこの回で扱う。
...
5. コマ3 の `codegen_Assign()` が左辺に `self.codegen_lval()` を使っていることを確認する（`*p = 20;` はこれで動く）
...
```bash
python3 scaffold/test_runner.py sessions/07_lvalue_rvalue
```
