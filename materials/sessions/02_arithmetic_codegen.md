...
1. `codegen_Num` が実装例として埋まっていることを確認する（`li a0, <val>`）。`codegen()` / `gen_stmt()` の `match` ディスパッチ、一時値の退避に使う `_push_a0()` / `_pop_into(reg)`、`gen_func()` の呼び出し順もあらかじめ書かれている
2. `_emit_func_prologue(name, frame_size)` と `_emit_func_epilogue(frame_size)` を埋め、関数の枠（`.globl`・関数ラベル・`ra`/`s0` の退避と復元・`sp` の戻し・`ret`）を出せるようにする
3. `_emit_func_body(body)` と `gen_stmt_Return(node)` を埋め、`return <式>;` の値が `a0` に残るようにする（ここまでで `return 42;` 相当が動く。どのテストでも必要）
4. `_codegen_binary_value(node, op)` を埋める（左辺を `codegen` → `self._push_a0()` → 右辺を `codegen` → `self._pop_into('a1')` → `<op> a0, a1, a0`。`sp` を直接動かすと積んでいる一時値の個数 `self._depth` がずれ、コマ6 のアラインメント調整が狂う）
5. `codegen_Add` / `codegen_Sub` / `codegen_Mul` を `_codegen_binary_value` へ委譲する（`add` / `sub` / `mul`）
6. `codegen_Div` / `codegen_Mod` を同じ形で委譲する（`div` / `rem`）
7. `codegen_Neg(node)` を埋める（`node.operand` を `codegen` してから `neg a0, a0`）
...
| ファイル | 内容 | 通る目安 | 期待値 |
|----------|------|----------|--------|
| `target.c` | `1 + 2 * 3` | 手順5 | 7 |
| `prec_div.c` | `(100 - 3 * 7) / 4 + 1` | 手順6 | 20 |
| `mod_div.c` | `100 / 4 + 1` | 手順6 | 26 |
| `nested.c` | `(1 + 2) * (3 + 4)` | 手順5 | 21 |
| `add.c` | `1 + 2` | 手順5 | 3 |
| `sub.c` | `100 - 79` | 手順5 | 21 |
| `mul.c` | 掛け算 | 手順5 | 対応する `.ans` を参照 |
| `div.c` | `17 / 5` | 手順6 | 3 |
| `mod.c` | `17 % 5` | 手順6 | 2 |
| `prec.c` | `2 + 3 * 4` | 手順5 | 14 |
| `paren.c` | カッコ付き算術式 | 手順5 | 対応する `.ans` を参照 |
| `complex.c` | `(100 - 3 * 7) / 4 + 1` | 手順6 | 20 |
| `div_round.c` | 負の数の `/` と `%`（`-7 / 2` は `-3`、`-7 % 2` は `-1`） | 手順7 | 69 |
