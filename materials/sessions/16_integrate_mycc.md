...
成果物はここで完成する。次は[コマ17（発表・振り返り）](17_demo_review.md)である。

コマ17 では新しい実装をしない。この回で完成させた `final/mycc.py` の設計を発表し、
生成アセンブリを1本読み下し、コマ1 までを逆順に振り返り、自己レビューで到達度を確かめる。
[選択制の発展課題](../../workbook/advanced/README.md)からどれを選ぶかも、その回で決める。

発展課題の中でも C 移植・セルフホスト（P1）は、動く Python 版を参照実装として、
Python の `dict` を C の連結リストへ、Python のクラスを C の `struct` へ写す
長期チャレンジである。選び方の目安はコマ17で扱う。

上の完成チェックリストは、そのままコマ17 の発表の準備にあたる。
とくに「生成アセンブリを1つ以上読んで説明できる」「苦労したバグとその直し方を説明できる」の2つは、
コマ17 で実際に人に向けて話す部分なので、ここで手を抜かないこと。
...
統合した `mycc.py` が出す最終的なアセンブリを1命令ずつ確認したいときは、[RV64 シミュレータ](../../tools/app.html?mode=run) に貼り付ける。
