...
| 連結リスト | `head = head->next` | 上の5つを組み合わせたデータ構造 |
...
## sizeof のコード生成 — コマ10 との差分

`sizeof(型名)` そのものはコマ10 で導入済みである。
「型情報からサイズを調べ、即値を `a0` に入れる」という形も、そこで決まっている。

この回で新しいのは、被演算子に `struct タグ名` が来る場合である。
...
コマ12 で作った構造体情報表を参照する必要がある。
そのため `size_of_ty_str()` には第2引数として `self._struct_defs` を渡す
（引数が1つ増えた経緯はコマ12 の原稿にある。名前は変わらない）。
...
`list_min.c` の S 式（この回の最初に確認したもの）で `n->next = NULL;` が
...
| 書き方 | 意味 |
|--------|------|
| `p = NULL;` | `p` はまだどこも指していない |
| `p != NULL` | `p` が有効なアドレスを持っている |
| `*p`（`p` が `NULL` のとき） | 実行時にクラッシュする |

`NULL` のポインタを間接参照してはいけない。
