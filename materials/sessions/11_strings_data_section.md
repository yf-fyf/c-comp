...
```bash
python3 scaffold/parse_viewer.py sessions/11_strings_data_section/tests/printf_number.c
```
...
この回ではまだ拾えない。`"abc"[1]` のような書き方はコマ12 で扱えるようになる。
...
コマ7で作った関数呼び出しのコード生成がそのまま使える（`self._gen_call()` または `ND_CALL` ハンドラメソッド）。
...
`lib.h` には `printf` 以外にストリーム操作の宣言もあるが、それらはコマ12 で使う。
...
```bash
python3 sessions/11_strings_data_section/mycc.py sessions/11_strings_data_section/tests/printf_hello.c \
...
生成したアセンブリで `printf` の呼び出しと標準出力を1命令ずつ確認したいときは、[RV64 シミュレータ](../../tools/app.html?mode=run) に貼り付ける。
