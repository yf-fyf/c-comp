...
コマ9 では `&` と `*` を実装し、ポインタも整数もすべて8バイト値として扱った。
...
この言語に配列はない。連続した領域を扱う話（`malloc` と添字）はコマ10b で扱う。
...
コマ9 で `codegen_lval` と `codegen` を分けたのと同じ理由である。
...
`importlib` でコマ9 の `Codegen09` を継承した `Codegen10a` に、以下の機能を追加する。
...
1. スケルトンの `Codegen10a` が `Codegen09` を `importlib` で継承していることを確認する
...
```bash
python3 scaffold/test_runner.py sessions/10a_types
```
...
```bash
python3 sessions/10a_types/mycc.py sessions/10a_types/tests/load_store_ty.c \
...
添字 `p[i]`・ポインタ演算・`sizeof(型名)`・前置 `++` の型対応はコマ10b で扱う。
...
この回の完成形を含む OCaml 版参考実装が [`../../workbook/ocaml/README.md`](../../workbook/ocaml/README.md) にある。完成相当の実装なので、まず自分の実装方針を検討してから確認すること。
