...
このディレクトリには、コマ1〜15 の各回の完成形に相当する OCaml 版実装をまとめて置いている。
Python 実装に詰まったときに、別言語での書き方と比較するための言語横断ヒントである。

- `sessions/lectureNN.ml`: コマ NN の正解に相当する OCaml 実装
- `reference/`: 必修パート（コマ1〜15）の完成版リファレンス実装。
  **生成されるアセンブリは lecture15 と同一である**（テストで担保している）が、
...
| `layout.ml` | 型の大きさ・アラインメントと struct のレイアウト。表は不変な Map |
...
Python 版 `mycc.py` と `sessions/lecture15.ml` は、コード生成をしながらその場で型を計算する。
...
# コマ1: インタープリター
dune exec sessions/lecture01.exe -- ../sessions/01_interpreter/tests/add_mul.c

# コマ3: 変数・代入
dune exec sessions/lecture03.exe -- ../sessions/03_variables/tests/target.c

# コマ15: 統合版（final/tests も入力にできる）
dune exec sessions/lecture15.exe -- ../final/tests/f01_arith.c
```

## 出力アセンブリのコメント（mycc_ref）

`mycc_ref.exe` は、どの命令をどの生成関数が出したのかを示すコメントを付けて出力する。
コメントを外した出力（`--no-comments`）は `lecture15.exe` の出力とバイト単位で同じである。
...
- **コメントを出すのは OCaml 版の `mycc_ref` だけ**であり、`lecture15.exe` 自体は
  コメントなしで出力する
...
各回のテストに加えて、**等価性テスト**（`lecture15.exe` の出力 ==
...
`lecture15.exe` は表の走査順、`mycc_ref.exe` は定義順に出すためである。
...
コマ2 以降は `riscv64-linux-gnu-gcc` と `qemu-riscv64` が必要である
（無い場合は `docker/rv64/run.sh` 経由で実行する）。
コマ1 はインタープリターなので、`評価結果:` の印字を `.ans` と比べる。
...
| 実装 | 対応するコマ | テーマ |
|------|--------------|--------|
| `sessions/lecture01.ml` | 01_interpreter | AST インタープリター |
| `sessions/lecture02.ml` | 02_arithmetic_codegen | 算術式のコード生成 |
| `sessions/lecture03.ml` | 03_variables | 変数・代入・シンボルテーブル |
| `sessions/lecture04.ml` | 04_if_else | if / else |
| `sessions/lecture05.ml` | 05_loops | while / for / break / continue |
| `sessions/lecture06.ml` | 06_functions_recursion | 関数呼び出し・再帰 |
| `sessions/lecture07.ml` | 07_lvalue_rvalue | lvalue / rvalue と `&` / `*` |
| `sessions/lecture08.ml` | 08_types | 型表・型別 load/store・`char` の昇格と縮小 |
| `sessions/lecture09.ml` | 09_pointer_arith | ポインタ演算・添字・`sizeof(型名)` |
| `sessions/lecture10.ml` | 10_strings_data_section | 文字列リテラル・`.data`・`printf` の基本形 |
| `sessions/lecture11.ml` | 11_expr_walk_libc | 式の走査・`lib.h` の残りの外部関数 |
| `sessions/lecture12.ml` | 12_struct_malloc_list | struct / `.` / `->` / `sizeof` / `malloc` / 連結リスト |
