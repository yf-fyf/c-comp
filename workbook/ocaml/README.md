...
**コマ10・コマ11 は配列を正しく扱えない。** 配列名を値として評価するときに
ポインタへの読み替え（配列はその先頭要素のアドレスとして振る舞う）を実装しておらず、
配列の中身を読み込んでしまう。このため `a[i]` のアドレス計算が壊れ、
`sessions/10_types_arrays/tests/` の2件が失敗する（無限ループ・領域外アクセス）。
**コマ12 以降では修正されている**ので、配列の扱いを参照したいときは `koma12.ml` 以降を見ること。
Python 版にこの不具合はない。
