...
let ty_note ty =
  let name =
    ...
    | TyPtr _ -> "ポインタ"
    | TyArray _ -> "配列"
    | TyStruct { name = Some n; _ } -> "struct " ^ n
    | TyStruct _ -> "無名構造体"
    | TyUnknown n -> n
  in
...
let unop_name = function
  | Neg -> "Neg" | Not -> "Not" | BitNot -> "BitNot" | Addr -> "Addr" | Deref -> "Deref"
...
let emit_data_section () =
  ...
  Hashtbl.iter
    (fun s label ->
      emit_note (label ^ ":")
        (Printf.sprintf "intern_string: 文字列リテラル（codegen の StrLit が la で参照する）%s"
           (if String.length s > 20 then "" else Printf.sprintf " \"%s\"" (String.escaped s)));
  ...
  Hashtbl.iter
    (fun name -> function
      | Global { ty; init = Some v } ->
          ...
          emit_note (name ^ ":")
            (Printf.sprintf "emit_data_section: 初期値ありのグローバル変数 %s" (ty_note ty));
  ...
  Hashtbl.iter
    (fun name -> function
      | Global { ty; init = None } ->
          emit (Printf.sprintf "  .globl %s" name);
          emit_note (name ^ ":")
            (Printf.sprintf "emit_bss_section: 初期値なしのグローバル変数 %s" (ty_note ty));
