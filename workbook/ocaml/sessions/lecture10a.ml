(*
    コマ 10a: 型システムの導入 — 型表・型別 load/store・char の昇格と縮小

    コマ9 まではローカル変数表がオフセットしか覚えておらず、読み書きは
    すべて ld / sd だった。この回から変数表に型を持たせ、型サイズで
    lb/lw/ld・sb/sw/sd を選ぶ。

    添字 p[i]・ポインタ演算のスケーリング・sizeof(型名) は、この型の
    仕組みの上に載る話なので、コマ10b（lecture10.ml）で足す。
*)
...
(* 宣言時の型を offset と一緒に覚える。ロード・ストアの幅は、この型で決まる。
   コマ9 の (string, int) Hashtbl に型を足したのがこの表である。 *)
...
and type_of_expr = function
  ...
  (* 算術は常に int で行う。ポインタ + 整数がポインタ型になるのはコマ10b から。 *)
  | _ -> TyInt
...
and codegen = function
  ...
  | Binary { op; lhs; rhs; _ } ->
      ...
      | _ -> error "コマ10aで未対応の二項演算です")
  | e -> error ~line:(line_of_expr e) "コマ10aで未対応の式です"
...
let () =
  if Array.length Sys.argv < 2 then (
    prerr_endline "使い方: dune exec ./lecture10a.exe -- <source.c>";
    exit 1);
