(*
    コマ 11a: 文字列リテラルと .data セクション — intern_string, 文の走査, printf の基本形
    ...
    下にある文字列を取りこぼすので、そこを埋めるのがコマ11b（lecture11.ml）である。
*)
...
and codegen = function
  ...
  | Cond { cond; then_; else_; _ } ->
      ...
      codegen else_;
      emit (label_end ^ ":")
  | e -> error ~line:(line_of_expr e) "コマ11aで未対応の式です"
...
let () =
  if Array.length Sys.argv < 2 then (
    prerr_endline "使い方: dune exec ./lecture11a.exe -- <source.c>";
    exit 1);
