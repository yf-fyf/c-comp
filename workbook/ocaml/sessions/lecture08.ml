(*
   コマ8: 型システムの導入 — 型表・型別 load/store・char の昇格と縮小

   コマ7 まではローカル変数表がオフセットしか覚えておらず、読み書きは
   すべて ld / sd だった。この回から変数表に型を持たせ、型サイズで
   lb/lw/ld・sb/sw/sd を選ぶ。

   添字 p[i]・ポインタ演算のスケーリング・sizeof(型名) は、この型の
   仕組みの上に載る話なので、コマ9（lecture09.ml）で足す。
*)
...
(* 宣言時の型を offset と一緒に覚える。ロード・ストアの幅は、この型で決まる。
   コマ7 の (string, int) Hashtbl に型を足したのがこの表である。 *)
