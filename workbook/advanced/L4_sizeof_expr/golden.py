#!/usr/bin/env python3
"""L4 golden test — sizeof 式が動くこと + fixed17 が壊れないこと
...
def main():
    ...
    print("=== 1. sizeof 式なし(いまの mycc)では構文エラーになることを確認 ===")
    ok_plain, out_plain = run_tests(compiler, tests)
    if ok_plain:
        print("(すでに sizeof 式が実装されているコンパイラのようです)")
    else:
        print("sizeof 式なし: FAIL —『sizeof は sizeof(型名) 形式のみ』。期待どおり")

    print()
    print("=== 2. sizeof 式ありで全テストが通る ===")
    ...
    if not ok_fixed:
        print("\n従来の sizeof(型名) 形式を壊していないか確認する。")
        return 1

    print()
    print("sizeof 式が動き、既存のテストも壊れていない!")
    return 0
