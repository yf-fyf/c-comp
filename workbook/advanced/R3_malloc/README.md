...
`malloc` を自分で書きます。バンプ割り当て → ヘッダ付きの再利用まで作り、
コマ12 の連結リストを自前アロケータの上で動かします。

## この回の位置づけ

| 項目 | 内容 |
|------|------|
| 必須の前提 | コマ12（`malloc` と連結リスト）まで、およびコマ15。`check.py` が完成した `final/mycc.py` で `mymalloc.c` と `tests/*.c` をコンパイルするので、コマ15 が無いと1件も実行できない |
| 推奨の前提 | — |
...
`check.py` が使う土台のコンパイラは、環境変数 `R3_COMPILER` で差し替えられます
