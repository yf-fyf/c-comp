...
`advanced/<回ID>_<内容>/` の形で、`handout.pdf`・`README.md`・スケルトン・`check.py`（多くは `golden.py`）が
そのディレクトリの中に閉じている。
...
| F4 | `F4_parser_decl/` | 再帰下降③: 宣言・型・typedef（黒箱の完全置き換え） | 1 | F3 |
...
| L1 | `L1_struct/` | 構造体の代入・ネスト（値渡しは発展課題） | 1 | コマ12 |
| L2 | `L2_initializer/` | 初期化子リスト（`int a[3] = {1,2,3}`、`char s[] = "hi"`） | 1 | コマ14 |
...
- 完了条件は各トピックの `README.md` と `handout.pdf` に書いてある
...
- **F1 以降**は積み上げ式（F1 → F2 → F3 → F4）。最終目標は、自作の Lexer / Parser で `final/tests`（fixed15）を通すこと
...
bubble           311   227   27.0%    23209   16731   27.9%
fib_rec           78    71    9.0%    73994   68074    8.0%
loop_sum          70    43   38.6%     7840    4030   48.6%
matmul           386   270   30.1%    27321   19095   30.1%
strops           244   170   30.3%    77128   50075   35.1%
合計            1089   781   28.3%   209492  158005   24.6%
```
...
ラッパーで生成を差し替える方式で、**fixed15 が通り続けること**が安全網になる。
