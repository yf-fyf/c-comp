...
型・宣言・`struct` 定義・関数を実装して `parse_program` を完成させます。
講義の全テスト入力（約100本の `.c`）で AST が スキャフォールド と完全一致したら、
Lexer（F1）とあわせてブラックボックスの完全な置き換えを達成します。

この回の主題は「同じ型の文法でも、書かれた位置（引数・変数・戻り値）で
許される型が違う」ことです。スキャフォールド の scalar / obj / ret の3分類を自分で書きます。
...
```bash
python3 myparser.py ../../sessions/12_struct_malloc_list/tests/list_min.c
```
