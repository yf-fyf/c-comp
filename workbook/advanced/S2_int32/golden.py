#!/usr/bin/env python3
"""S2 golden test — 32bit で折り返すこと + fixed15 が壊れないこと
...
ensure_base(compiler, "SEMCC_COMPILER")
...
def main():
    ...
    print("=== 1. 64bit のまま(いまの mycc)では落ちることを確認 ===")
    ok_plain, out_plain = run_tests(compiler, tests)
    tail = out_plain.strip().splitlines()[-2:-1]
    print("\n".join(tail) if tail else out_plain.strip()[-200:])
    if ok_plain:
        print("(すでに32bit演算が実装されているコンパイラのようです)")
    else:
        print("→ 32bit で折り返さないので桁あふれのテストが落ちる。期待どおり")

    print()
    print("=== 2. 32bit 化して全テストが通る ===")
    ...
    if not ok_fixed:
        print("\nアドレス計算まで narrow していないか確認する(a[i] が壊れやすい)。")
        return 1

    print()
    print("int が32bitで折り返し、既存のテストも壊れていない!")
    return 0
