#!/usr/bin/env python3
"""O7 golden test — 正しさと「動的だけ減る」ことを確かめる

使い方:
    python3 golden.py
    OPT_COMPILER=... OPT_ANSWERS=... python3 golden.py

この回は**静的命令数がほとんど変わらない**。
効果があるかどうかは動的命令数で判定する。
"""
...
def main():
    ...
    print(f"{'ベンチマーク':<12} {'静的:前':>8} {'後':>7} {'差':>5} "
          f"{'動的:前':>10} {'後':>10} {'削減':>7}")
    print("-" * 66)
    tb = ta = db = da = 0
    with tempfile.TemporaryDirectory() as tmp:
        for csrc in sorted(BENCH.glob("*.c")):
            ...
            db += d0
            da += d1
            print(f"{csrc.stem:<12} {s0:>8} {s1:>7} {s1 - s0:>5} "
                  f"{d0:>10} {d1:>10} {(d0 - d1) * 100 / d0:>6.1f}%")
    print("-" * 66)
    print(f"{'合計':<12} {tb:>8} {ta:>7} {ta - tb:>5} "
          f"{db:>10} {da:>10} {(db - da) * 100 / db:>6.1f}%")
    ...
    print()
    print("静的命令数はほとんど変わらないのに、動的命令数は減っている。")
    print("O1 で作った動的の物差しが無ければ、この最適化は「効果ゼロ」に見える。")
    return 0
