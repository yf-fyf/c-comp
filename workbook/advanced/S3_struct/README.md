...
`q = p;` で構造体をまるごとコピーできるようにします。
ポインタ・フィールドを持つ構造体やポインタ経由の代入も動くようにします。
...
| コマ数 | 1 |
| 仕様との関係 | **仕様の内側**（`language_spec.md` の対応表がこの構文に付ける「仕様外」区分は、標準トラックの実装対象外という別軸の区分）。`language_spec.md` は「struct 変数は宣言できるが、struct 値の代入・実引数・戻り値は不可」と定めているのに、実装は `q = p;` をエラーにせず受理して誤った答えを返す。判定基準どおりの **S 系**である |
