...
| 項目 | 内容 |
|------|------|
| 必須の前提 | コマ13（構造体）まで。短絡の考え方そのものは関数呼び出しまでで足りるが、`tests/null_guard.c` が `struct` を使う（短絡が要る代表例の `p != 0 && p->val > 0` がポインタとフィールド参照そのものなので避けられない）。完了条件のうち `golden.py` は完成した `final/mycc.py` を土台に `fixed17` を回すので、やり切るにはコマ16 も要る |
| 推奨の前提 | — |
