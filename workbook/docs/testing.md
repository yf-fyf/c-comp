...
`SKIP` になるのは、コマ14 の `math_util.c` / `stat_lib.c` のように
**単体では実行せず他のテストと一緒にコンパイルする補助ソース**だけである。
補助ソースかどうかは、同じディレクトリのどれかの `.files` から参照されているかで判定する。

それ以外の `.c` に `.ans` が無いときは `SKIP` ではなく `FAIL` になる。
期待値の置き忘れを「実行されなかったので成功」と読み違えないための扱いである。
同じ理由で、**テストがあるのに PASS が 0 件のとき**も終了コードは 0 にならない。
...
foo.stderr  # 期待する標準エラー出力（必要な場合のみ）
...
`.stdout` / `.stderr` は、置いたときだけ突き合わせる。
`fdopen(2, "w")` で書いた出力のように、標準出力に出ないものを確認したいときは `.stderr` を置く
（例: `sessions/11_expr_walk_libc/tests/file_stream.stderr`）。
...
一緒にコンパイルされる補助ソース**だからである。テストランナーはこの `.files` の
記述から `stat_lib.c` を補助ソースと判定し、直接拾ったときは `SKIP` にする
（前述のとおり正常な挙動）。逆に言えば、どの `.files` からも参照されていない
`.c` に `.ans` を置き忘れると `FAIL` になる。
...
| AST の構造が分からない（優先順位・結合の向き） | 目視では判断しづらい | [AST ビジュアライザ](../../tools/app.html?mode=build) にソースを貼って構文木と S 式を確認する |
| 生成されたアセンブリの動きを目で追いたい | 静的に読むだけでは1命令ごとのレジスタ・スタックの変化が分からない | [RV64 シミュレータ](../../tools/app.html?mode=run) にアセンブリを貼って1命令ずつ実行する |
