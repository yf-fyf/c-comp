...
> 全17コマ構成（実装のコマ1〜16 + 発表・振り返りのコマ17。番号は 01〜17 の連番）
...
| **発展**（任意） | 標準トラック（コマ17 まで）を終えたあと、選択制の発展課題（[`../advanced/`](../advanced/README.md)）に挑戦する |
...
| Phase 1 | 3〜12 | Python 版ミニコンパイラを完成させる |
| Phase 2 | 13〜16 | Python 版標準機能を完成させる |
| 最終デモ | 17 | 発表・振り返り（新しい実装はしない） |

Phase 1 の終わり（コマ12）で「小さいが動く Python 版コンパイラ」が一度完成する。
ここで到達感を作ってから、Phase 2 で構造体などの重い機能に進む構成である。

コマ17 だけは新しいコードを書かない。既にある自分の `final/mycc.py`・`fixed17`・
...
| 7 | 関数呼び出し・再帰 | 再帰フィボナッチが動く |
| 8 | lvalue / rvalue の概念 + `&` / `*` | `codegen()` と `codegen_lval()` を別関数として設計・実装できる |
| 9 | 型システムの導入 | 変数の型を覚え、`char` / `int` / ポインタで読み書き命令を選べる |
| 10 | ポインタ演算とスケーリング | `malloc(sizeof(int) * 5)` で確保した領域への `a[i]` と `*(p + 2)` のポインタ演算が動く |
| 11 | 文字列リテラルと `.data` セクション | `printf("hello\n")` が動く |
| 12 | 式の走査と libc 活用 + 総合確認 | どこに書いた文字列リテラルも `.data` に出る。ミニコンパイラの完成を確認する |
| 13 | 構造体とヒープ（struct / `.` / `->` / `sizeof` / `malloc` / 連結リスト） | `struct Point` のメンバアクセスと、連結リストの構築・走査が動く |
...
| 17 | 発表・振り返り | 自分の設計を10分で説明でき、生成アセンブリを1本読み下せる。自己レビューで次の一手を決められる |
...
コマ16 で Python 版が完成したら、**次は [コマ17（発表・振り返り）](https://yf-fyf.github.io/c-comp/sessions/17_demo_review/) である。**
新しい実装は無い。完成した自分の `final/mycc.py` を説明できる形にし、
そのうえで次に何をやるかを決める回で、標準トラックはそこで終わる。

| 行き先 | 内容 |
|--------|------|
| [コマ17](https://yf-fyf.github.io/c-comp/sessions/17_demo_review/) | 設計の発表・生成アセンブリの読み下し・逆順の振り返り・自己レビュー。**発展課題の選び方もこの回で扱う** |
| [`../advanced/`](../advanced/README.md) | 選択制の発展課題 26 トピック。一部はコマ16 を待たずに始められる（F0 はコマ2、B 系列・S1 はコマ7 から）。C 移植・セルフホスト（P1）は複数回に渡る最難関トピック |

どのトピックを選ぶかの判断材料はコマ17 の資料にまとめてあるので、ここでは繰り返さない。
