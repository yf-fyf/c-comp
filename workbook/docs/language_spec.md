...
| [実行時の意味](#semantics) | サイズ・アラインメント、評価順序、未定義動作 |
...
- 置換されるのは**コード中の識別子トークンだけ**である。文字列リテラル
  `"NAME"`、文字リテラル `'N'`、行コメント `// NAME` の中身は置換されない。
...
| 条件 `? :` | コマ4 | `sessions/04_if_else/tests/ternary.c` | 標準 | 選ばれた枝だけを評価する |
...
| 機能 | 導入コマ | 代表テスト | 区分 | 備考 |
|------|---------|-----------|------|------|
| 型のサイズとアラインメント・struct レイアウト | コマ12 | `sessions/12_struct_malloc_list/tests/malloc_struct.c` | 標準 | サイズは `sizeof_test.c` |
...
| ビット演算・シフト | アラインメントは `/` と `*`、ハッシュは `%`、フラグは個別の int フィールド |
...
| `int` | 4 | 4 |
| ポインタ | 8 | 8 |
| `struct` | 下記 | 最大フィールドのアラインメント |

struct のフィールドは宣言順に配置し、各フィールドのオフセットはそのアラインメントへ
切り上げる。struct 全体のサイズは struct のアラインメントの倍数へ切り上げる。
...
- `?:` は条件を評価してから選ばれた枝だけを評価する(こちらは ISO C と同じ)。
...
`sizeof(void)` は書けない。
struct 値の引数・戻り値は、この `param` と `ret_type` を拡張して発展 L5 が追加する。
