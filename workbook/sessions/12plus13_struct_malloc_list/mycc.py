"""
コマ12+13: 構造体とヒープ（struct / . / -> / sizeof / malloc / 連結リスト。学生用スケルトン）

目標: struct のレイアウトを管理してメンバアクセスをコード生成し、
      sizeof(struct タグ) で求めたサイズを malloc に渡して連結リストを動かす。

実行方法:
    python3 sessions/12plus13_struct_malloc_list/mycc.py input.c \
...
class Codegen1213(prev.Codegen11):
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/12plus13_struct_malloc_list/mycc.py <source.c>",
              file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
    ...
    struct_defs = Codegen1213.parse_struct_defs(source)
    tokens = tokenize(source, filename)
    prog = parse(tokens)
    cg = Codegen1213(struct_defs)
    for node in prog:
