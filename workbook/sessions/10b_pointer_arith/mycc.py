"""コマ 10b: ポインタ演算とスケーリング（学生用スケルトン）。連続領域は malloc + sizeof で確保する。"""

import importlib.util
import sys
from pathlib import Path

_PREV = Path(__file__).resolve().parents[1] / '10a_types' / 'mycc.py'
_SPEC = importlib.util.spec_from_file_location('_session10a', _PREV)
...
class Codegen10b(prev.Codegen10a):
    ...
    def codegen(self, node: Node) -> None:
        match node.kind:
            ...
            case _:
                raise RuntimeError(f'codegen: コマ10bで未対応の式です (kind={node.kind!r})')
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/10b_pointer_arith/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
