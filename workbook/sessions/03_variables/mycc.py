"""
コマ 3: コード生成② — 変数・代入・シンボルテーブル（学生用スケルトン）

目標: コマ 2 のコード生成器を継承し、ローカル変数の宣言・参照・代入を追加する。
"""
...
_PREV = Path(__file__).resolve().parents[1] / '02_arithmetic_codegen' / 'mycc.py'
_SPEC = importlib.util.spec_from_file_location('_session02', _PREV)
...
class Codegen03(prev.Codegen02):
    ...
    def codegen(self, node: Node) -> None:
        match node.kind:
            ...
            case _:
                raise RuntimeError(f'codegen: コマ3で未対応の式です (kind={node.kind!r})')
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/03_variables/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
