"""コマ 11a: 文字列リテラルと .data セクション（学習者用スケルトン）。"""
...
class Codegen11a(prev.Codegen10):
    ...
    def collect_strings_expr(self, node: Node) -> None:
        ...
        # （`case _: pass`）。この取りこぼしを埋めるのがコマ11b である。
        match node.kind:
            case 'Str':
    ...
    def codegen(self, node: Node) -> None:
        match node.kind:
            ...
            case _:
                raise RuntimeError(f'codegen: コマ11aで未対応の式です (kind={node.kind!r})')
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/11a_strings_data_section/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
