"""
コマ 15: コード生成⑬ — 複数ファイル・前処理 (#include / #define)（学生用スケルトン）
...
実行方法:
    python3 sessions/15_preprocess_multifile/mycc.py main.c lib.c \
...
_PREV = Path(__file__).resolve().parents[1] / '14_globals_scope' / 'mycc.py'
_SPEC = importlib.util.spec_from_file_location('_session14_mycc', _PREV)
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/15_preprocess_multifile/mycc.py <source.c> [...]", file=sys.stderr)
        sys.exit(1)
