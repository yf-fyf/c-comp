"""
コマ13: 構造体とヒープ（struct / . / -> / sizeof / malloc / 連結リスト。学生用スケルトン）

目標: struct のレイアウトを管理してメンバアクセスをコード生成し、
      sizeof(struct タグ) で求めたサイズを malloc に渡して連結リストを動かす。

実行方法:
    python3 sessions/13_struct_malloc_list/mycc.py input.c \
...
_PREV = Path(__file__).resolve().parents[1] / '12_expr_walk_libc' / 'mycc.py'
_SPEC = importlib.util.spec_from_file_location('_session12', _PREV)
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/13_struct_malloc_list/mycc.py <source.c>",
              file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
