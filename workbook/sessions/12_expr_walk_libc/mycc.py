...
_PREV = Path(__file__).resolve().parents[1] / '11_strings_data_section' / 'mycc.py'
...
class Codegen12(prev.Codegen11):
    # コマ11 の dispatch は Str / Assign / Call だけを見ていた。ここでは残りの
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/12_expr_walk_libc/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
