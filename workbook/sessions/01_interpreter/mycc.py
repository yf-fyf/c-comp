"""
コマ 1: AST インタープリター（学生用スケルトン）

目標: AST を受け取り、式を評価して整数値を返す eval_ast() を実装する。
      コード生成をせず、Python の整数演算で直接計算する。

実行方法:
    python3 sessions/01_interpreter/mycc.py input.c
"""
...
# ---- C の除算・剰余のヘルパー ----
# C では / はゼロ方向に切り捨て、% の符号は被除数と同じ(言語仕様: 整数除算の
# 丸め方向)。Python の // は「本来の計算方法(数学的な除算)」である床除算
# (floor division)であり、負数が絡む場合はゼロ方向切り捨てと丸め方向が
# 異なる(例: -7 // 2 は -4 だが、C の -7 / 2 は -3)。そのため // は使わず、
# 以下のようにゼロ方向切り捨てを明示的に実装する。
...
def eval_ast(node: Node) -> int:
    match node.kind:
        ...
        case _:
            raise RuntimeError(f"eval_ast: コマ1で未対応の式です (kind={node.kind!r})")


def run_main(prog: list[Node]) -> int:
    """コマ1の対象である main の return 式を eval_ast に渡す。"""
    for node in prog:
...
def main():
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/01_interpreter/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
