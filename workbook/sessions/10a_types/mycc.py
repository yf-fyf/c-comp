"""コマ 10a: 型システムの導入（学生用スケルトン）。int / char / ポインタを型サイズで扱う。"""

import importlib.util
import sys
from pathlib import Path

_PREV = Path(__file__).resolve().parents[1] / '09_lvalue_rvalue' / 'mycc.py'
...
class Codegen10a(prev.Codegen09):
    @classmethod
    ...
    def codegen(self, node: Node) -> None:
        match node.kind:
            ...
            case _:
                raise RuntimeError(f'codegen: コマ10aで未対応の式です (kind={node.kind!r})')
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/10a_types/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
