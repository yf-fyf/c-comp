"""
コマ 5: コード生成③ — if/else・比較演算子（学生用スケルトン）

目標: コマ 4 のコード生成器を継承し、比較演算子、複文、if/else、return ラベルを追加する。
"""
...
_PREV = Path(__file__).resolve().parents[1] / '04_variables' / 'mycc.py'
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/05_if_else/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
