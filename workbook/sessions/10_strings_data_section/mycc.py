"""コマ 11: 文字列リテラルと .data セクション（学習者用スケルトン）。"""

import importlib.util
import sys
from pathlib import Path

_PREV = Path(__file__).resolve().parents[1] / '09_pointer_arith' / 'mycc.py'
_SPEC = importlib.util.spec_from_file_location('_session09', _PREV)
...
class Codegen11(prev.Codegen10):
    ...
    def collect_strings_expr(self, node: Node) -> None:
        # この回の dispatch は、文字列リテラルそのものと、文字列リテラルを直接
        # 抱えうる2種類の式（代入・関数呼び出し）だけを辿る。二項演算・単項演算・
        # 添字・三項演算子の下にある文字列は、まだ拾えないまま素通りする
        # （`case _: pass`）。この取りこぼしを埋めるのがコマ11 である。
        match node.kind:
            case 'Str':
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/10_strings_data_section/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
