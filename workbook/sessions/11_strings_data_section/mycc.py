...
_PREV = Path(__file__).resolve().parents[1] / '10_pointer_arith' / 'mycc.py'
...
class Codegen11(prev.Codegen10):
    ...
    def collect_strings_expr(self, node: Node) -> None:
        ...
        # （`case _: pass`）。この取りこぼしを埋めるのがコマ12 である。
        match node.kind:
            case 'Str':
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/11_strings_data_section/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
    filename = sys.argv[1]
