"""
コマ 3: コード生成① — 算術式 → RV64 アセンブリ（学生用スケルトン）

目標: 整数定数・四則演算・剰余・括弧・単項マイナスを含む式を
      RV64 アセンブリに変換する Codegen03 を実装する。

実行方法:
    python3 sessions/03_arithmetic_codegen/mycc.py input.c \
...
def main() -> None:
    if len(sys.argv) < 2:
        print("使い方: python3 sessions/03_arithmetic_codegen/mycc.py <source.c>", file=sys.stderr)
        sys.exit(1)
