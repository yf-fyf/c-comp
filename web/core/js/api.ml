...
(* 文・式と命令の対応（A3）。参考実装は「前処理後ソースのバイト範囲 → 出力の行番号」で返すので、
   ここで範囲を元ソースの UTF-16 位置へ写す（AST ノードの sourceRanges と同じ土俵に載せる）。
   マクロ展開や include 由来で元ソースへ写せない文・式は落とす。

   map_span は preprocess をもう 1 回走らせて作るので、stmtMap と exprMap で使い回す
   （compile 1 回につき前処理は 2 回まで）。 *)
let span_mapper source =
...
let span_map_json map_span kind (spans : Refcomp.Emitter.stmt_span list) =
  JList
    (List.filter_map
       (fun (s : Refcomp.Emitter.stmt_span) ->
         if s.s_kind <> kind then None
         else
...
let compile_json source comments =
  try
    ...
    | Ok (text, spans) ->
        let map_span = span_mapper source in
        JObj
          [ ("ok", JBool true);
            ("text", JStr text);
            ("stmtMap", span_map_json map_span Refcomp.Emitter.Stmt spans);
            ("exprMap", span_map_json map_span Refcomp.Emitter.Expr spans) ]
