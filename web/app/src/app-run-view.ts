...
export function initRunView(opts: RunViewOptions): RunView {
  ...
  const select = $<HTMLSelectElement>("run-example-select");

  /** 参照実装の出力は、注意書きに同意してから出す */
  function addGatedGroup(gated: SimExample[], byLabel: Map<string, SimExample>): void {
    const label = $("run-example-label");
  ...
  async function start(wanted: string | null): Promise<void> {
    ...
    if (gated.length > 0) addGatedGroup(gated, byLabel);
