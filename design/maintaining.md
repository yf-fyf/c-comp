...
│   ├── sessions/          # 通常回（コマ1〜16。07 は欠番）の原稿 NN_xxx.md
...
| 「black box」の訳語 | ブラックボックス（カタカナの借用語） | 黒箱 | 「黒箱」は英語 "black box" の逐語訳(calque)で、日本語の技術文書としては不自然。T98 で `materials/`・`workbook/` 配下の既知の出現は置換済み。`tools/doc_check_allowlist.yaml` の `pending` エントリに、対象外だった残存箇所を記録してある |
