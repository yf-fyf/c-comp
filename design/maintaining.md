...
テストファイルの本数・コード行数など、修正のたびに増減しうる派生的な数値は、
教材本文（`materials/`・`workbook/`）に新たに書き込まない。数量に触れたいときは
「対応するテストで確認できる」のように、実測し直さなくても正しさが保たれる表現へ言い換える。

対象外: コマ番号・トピック数など教材の骨格を表す数値（書いてよい。ただしトピック数の
ように複製される値は、書く前に実体のディレクトリ数と突き合わせる）。
...
非公開リポジトリ `c-comp-design` のタスク管理が使う内部タスク番号（`T49` 等）は
非公開側だけの内部参照であり、公開リポジトリ `c-comp` の読者が
