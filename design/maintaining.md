...
| スキャフォールド／scaffold（`workbook/` 直下） | 地の文では「スキャフォールド」（カタカナ）。ディレクトリ名・コマンド例など実体を指す場合は `scaffold/`（ローマ字、変更しない） | 地の文での英字 `scaffold` | `workbook/advanced/` 配下の表記統一は別タスクで扱う（本表の対象外） |

`workbook/advanced/` と `materials/advanced/` 側の用語・表記（全角/半角括弧、
「発展 XN」/「発展課題 XN」の呼称、B ファミリの呼称、「（選択制）」の有無、
スキャフォールド／scaffold のうち advanced 配下のもの）は、この節ではなく
別タスクで別途統一する。
