...
- AST ノードの型定義
- テストランナー
- `lib.h`（`printf`・`fopen` 系・`malloc`・`exit` の宣言と `NULL` の定義。
  提供する宣言の一覧はここには列挙しない(規範は `workbook/docs/language_spec.md` の「標準ライブラリ」節。実体 `workbook/scaffold/lib.h` との一致は `tools/check_docs.py` の `libh` チェックが保証する)。
...
（`fixed17` 全通が目安）。コマ15 でこれを完成させれば主目標は達成である。

その先は必修ではなく、選択制の発展課題（`workbook/advanced/`、全28トピック）として扱う。
...
### Phase 1: Python 版ミニコンパイラを完成させる（コマ2〜11）
...
| コマ | 導入する設計 |
|------|-------------|
| 2 | インタープリターの構造を `emit()` に置き換える（構造を変えずに出力先だけ変える） |
| 3 | ローカル変数のスタックオフセット管理。関数本体（`Block` ノード）からの宣言収集とフレームサイズ計算を `_reset_func_state()` にまとめる |
| 4 | ラベル生成。Python の `str` でラベルを管理する。三項演算子 `?:`（`ND_COND`）も同回で導入する |
| 5 | ループ開始/終了ラベル、`break` / `continue` のジャンプ先管理（ラベルスタック）。前置 `++`/`--`（`preinc`/`predec`）も同回で導入する |
| 6 | `a0`〜`a7` での引数受け渡し、引数のスタック退避、`call` 前の16バイトアラインメント（`_push_a0()` / `_pop_into()` が数える一時値の個数（`_depth`）が奇数のときの padding。引数の個数ではなく、call 時点で外側の式が積んでいる一時値の個数で決まる） |
| 7 | **`codegen()` / `codegen_lval()` の分離**。全ポインタ操作の根幹で、この設計なしに先へ進めない |
| 8 | `ty_str` 文字列による型サイズ管理（`size_of_ty_str` / `elem_ty_str`）と型別のロード / ストア |
| 9 | 型サイズを尺度に使うポインタ演算・添字・`sizeof(型名)` |
| 10 | `.data` セクションへの出力とラベル参照。`#include "lib.h"` を使い始める |
| 11 | 文字列を集める走査を式全体へ広げる。`lib.h` の残りの外部関数を使う |

> **アラインメントの注意点（コマ3 以降）**: 16バイトアラインメント違反は無音クラッシュを招く。
> `align_to(n, 16)` ヘルパーをスケルトンで提供し、アラインメント計算を自動化する
> （コマ3 でフレームサイズ、コマ6 で `call` 前のスタックに適用する）。
>
...
### Phase 2: Python 版標準機能を完成させる（コマ12〜15）

| コマ | 導入する設計 |
|------|-------------|
| 12 | 構造体情報（サイズ・フィールドオフセット）を `self._struct_defs` の `dict` で管理する。`sizeof(type)` は `size_of_ty_str()` でコンパイル時に定数化する |
| 13 | `self._locals` → `self._globals` の2段階探索で隠蔽を実現する |
| 14 | スキャフォールドの `preprocess()` で展開する。`#define` の自前実装は発展課題扱い |
| 15 | 新しい機能は入れない。統合と可読性の作り込みに充てる |

> **コマ15 の重要性**: 標準トラックの主目標はここで完了させる。
...
設計の発表、生成アセンブリの読み下し、コマ15 からコマ1 への逆順の振り返り、
自己レビューのルーブリック、発展課題の選び方の5つで構成する。

分離した理由は、コマ15 が統合・総合テスト・バグ切り分け・コードレビューで既に満杯であり、
...
└── final/tests/            # fixed17（f01〜f17）。コマ1〜15 の全機能をまとめて確認
```
...
学習者に手計算させない。適用箇所はコマ3（フレームサイズ）とコマ6（`call` 前の `sp`）の2つ。

さらにコマ3 では、意図的にアラインメントを崩したコードを生成して
...
- `include_libh` の導入コマは **10** である。`#include "lib.h"` の行自体はコマ9 の
  テストにも現れるが、そこでは「そう書くもの」として置くだけで、`#include` が
  何をするかはコマ10 で初めて説明する（コマ9 の原稿がそう明記している）。
  台帳の導入コマは「原稿が初めて説明する回」なので、実初出の行の位置ではなく 10 を取る。
...
| `codegen_a0_contract` | `codegen(node)` の約束（結果を `a0` に残す） | 2 | `eval_ast_interpreter`, `rv64_asm_handwritten` |
| `func_prologue_epilogue` | 関数プロローグ / エピローグ | 2 | `rv64_asm_handwritten` |
| `int_type` | `int` 型 | 2 | `codegen_a0_contract` |
| `int_literal` | 整数リテラル | 2 | `codegen_a0_contract` |
| `arith_ops` | 算術 `+ - * / %`・単項 `-`・括弧 | 2 | `codegen_a0_contract` |
| `div_mod_rounding` | `/` `%` の丸め（0 方向へ切り捨て） | 2 | `arith_ops` |
| `stack_temporaries` | スタック経由の中間値退避（`_push_a0` / `_pop_into`） | 2 | `codegen_a0_contract` |
| `return_stmt` | `return 式;` | 2 | `codegen_a0_contract`, `func_prologue_epilogue` |
| `main_function` | `int main()` | 2 | `return_stmt` |
| `local_var_decl` | ローカル変数宣言（初期化子なし） | 3 | `int_type` |
| `stack_frame_offsets` | スタックフレームと変数オフセット管理 | 3 | `func_prologue_epilogue` |
| `alloc_local` | `alloc_local()` による領域割り当て | 3 | `stack_frame_offsets` |
| `decl_collection` | 関数本体からの宣言収集とフレームサイズ計算 | 3 | `alloc_local` |
| `frame_align_16` | フレームサイズの16バイト整列（`align_to`） | 3 | `stack_frame_offsets` |
| `lvalue_rvalue_distinction` | rvalue と lvalue の区別 | 3 | `codegen_a0_contract` |
| `codegen_lval_var` | `codegen_lval()`（`'Var'` のみの最小形） | 3 | `lvalue_rvalue_distinction`, `alloc_local` |
| `var_reference` | 変数参照（アドレス取得 → `ld`） | 3 | `codegen_lval_var` |
| `assign_op` | 代入 `=` | 3 | `codegen_lval_var`, `stack_temporaries` |
| `gen_stmt_dispatch` | 文の生成 `gen_stmt()` と式との分離 | 3 | `codegen_a0_contract` |
| `expr_stmt` | 式文 | 3 | `gen_stmt_dispatch` |
| `comparison_ops` | 関係 `< > <= >=` | 4 | `arith_ops` |
| `equality_ops` | 等値 `== !=` | 4 | `arith_ops` |
| `bool_result_int01` | 比較結果が int の `0` / `1` | 4 | `comparison_ops`, `equality_ops` |
| `label_generation` | 一意ラベルの生成（`new_label()`） | 4 | `func_prologue_epilogue` |
| `if_else` | `if` / `else` / `else if` | 4 | `label_generation`, `gen_stmt_dispatch` |
| `block_stmt` | ブロック `{ }` | 4 | `gen_stmt_dispatch` |
| `nested_block_no_scope` | 入れ子ブロックが新しいスコープを作らない | 4 | `block_stmt`, `local_var_decl` |
| `ternary_op` | 条件 `? :` | 4 | `label_generation` |
| `common_epilogue` | 共通エピローグラベルへの合流 | 4 | `label_generation`, `return_stmt` |
| `while_stmt` | `while` | 5 | `label_generation`, `gen_stmt_dispatch` |
| `for_stmt` | `for` | 5 | `while_stmt` |
| `break_continue` | `break` / `continue` | 5 | `while_stmt` |
| `loop_label_stack` | ループラベルスタックによる飛び先管理 | 5 | `break_continue` |
| `prefix_incr_decr_int` | 前置 `++` `--`（int） | 5 | `assign_op` |
| `empty_stmt` | 空文 `;` と `for` の3式の省略 | 5 | `gen_stmt_dispatch` |
| `func_definition` | 関数定義 | 6 | `func_prologue_epilogue`, `gen_stmt_dispatch` |
| `func_params` | 仮引数の受け取りとスタック退避 | 6 | `func_definition`, `alloc_local` |
| `func_call` | 関数呼出し `f(args)` | 6 | `func_definition` |
| `rv64_calling_convention` | RV64 呼出し規約（`a0`〜`a7`・`ra`） | 6 | `func_call` |
| `call_stack_align_16` | `call` 前のスタック16バイト整列 | 6 | `rv64_calling_convention`, `frame_align_16` |
| `recursion` | 再帰呼び出し | 6 | `func_call` |
| `func_prototype` | プロトタイプ宣言 | 6 | `func_definition` |
| `mutual_recursion` | 相互再帰 | 6 | `func_prototype`, `recursion` |
| `codegen_lval_split` | `codegen()` / `codegen_lval()` の分離（`'Deref'` へ拡張） | 7 | `codegen_lval_var`, `lvalue_rvalue_distinction` |
| `pointer_type` | ポインタ型 `T *`（単段） | 7 | `int_type` |
| `addr_of` | アドレス取得 `&` | 7 | `codegen_lval_split` |
| `deref` | 間接参照 `*` | 7 | `codegen_lval_split`, `pointer_type` |
| `assign_through_pointer` | ポインタ経由の書き込み `*p = v;` | 7 | `deref`, `assign_op` |
| `pointer_param` | ポインタ引数 | 7 | `addr_of`, `func_params` |
| `void_return_type` | 戻り値型 `void` と `return;` | 7 | `func_definition`, `common_epilogue` |
| `char_type` | `char` 型 | 8 | `int_type` |
| `char_literal` | 文字リテラル `'a'` | 8 | `char_type` |
| `type_table_ty_str` | ローカル変数表の型情報（`ty_str`） | 8 | `pointer_type`, `char_type` |
| `type_sizes` | 型サイズ（`int` 4 / `char` 1 / `T *` 8） | 8 | `type_table_ty_str` |
| `size_of_ty_str` | `size_of_ty_str()` | 8 | `type_sizes` |
| `elem_ty_str` | `elem_ty_str()`（指し先の型） | 8 | `type_table_ty_str` |
| `load_store_by_type` | 型に応じたロード / ストア（`_load_ty` / `_store_ty`） | 8 | `type_sizes`, `deref` |
| `char_promotion` | `char` の昇格と縮小（`lb` / `sb`） | 8 | `load_store_by_type`, `char_type` |
| `sizeof_typename` | `sizeof(型名)`（`int` / `char` / ポインタ） | 9 | `size_of_ty_str` |
| `pointer_arith` | ポインタ ± int（尺度は指し先型） | 9 | `elem_ty_str`, `size_of_ty_str` |
| `subscript` | 添字 `p[i]`（`*(p + i)` の略記） | 9 | `pointer_arith`, `deref` |
| `multi_level_pointer` | 多段ポインタ `int **` | 9 | `elem_ty_str` |
| `malloc_call` | `malloc` によるヒープ確保 | 9 | `func_call`, `pointer_type` |
| `prefix_incr_pointer` | 前置 `++`（ポインタ、型対応） | 9 | `prefix_incr_decr_int`, `pointer_arith` |
| `main_argv` | `int main(int argc, char **argv)` | 9 | `multi_level_pointer`, `func_params` |
| `string_literal` | 文字列リテラルとエスケープ | 10 | `char_type` |
| `data_section` | `.data` セクションと `.text` の分離 | 10 | `rv64_asm_handwritten` |
| `string_label` | 文字列ラベルの採番と参照 | 10 | `data_section`, `label_generation` |
| `codegen_str` | `ND_STR` のコード生成（先頭アドレスを `a0` に） | 10 | `string_label` |
| `varargs_call` | 可変長引数の外部プロトタイプと呼出し | 10 | `rv64_calling_convention` |
| `printf_call` | `printf` 呼び出し | 10 | `varargs_call`, `string_literal` |
| `include_libh` | `#include "lib.h"` による外部宣言の取り込み | 10 | `func_prototype` |
| `stdout_test` | 標準出力テスト（`tests/foo.stdout`） | 10 | `printf_call` |
...
| `sizeof_struct` | `sizeof(struct タグ)` | 12 | `sizeof_typename`, `size_of_ty_str_struct` |
| `malloc_struct` | 構造体のヒープ確保 | 12 | `malloc_call`, `sizeof_struct` |
| `void_ptr_conversion` | `void *` と `T *` の暗黙変換 | 12 | `malloc_call`, `pointer_type` |
| `self_referential_struct` | 自己参照構造体と前方宣言 | 12 | `struct_definition`, `pointer_type` |
| `struct_tag_scope` | struct タグ名の扱い | 12 | `struct_definition` |
| `null_macro` | `NULL`（`lib.h` の `#define NULL 0`） | 12 | `include_libh`, `pointer_type` |
| `linked_list_traversal` | 連結リストの構築と走査 | 12 | `self_referential_struct`, `member_access_arrow`, `while_stmt`, `null_macro` |
| `global_var` | グローバル変数 | 13 | `local_var_decl`, `data_section` |
| `bss_section` | `.bss` セクションへの出力 | 13 | `global_var` |
| `global_zero_init` | グローバル変数の 0 初期化保証 | 13 | `bss_section` |
| `global_address_la` | グローバル変数のアドレス取得 | 13 | `global_var`, `codegen_lval_split` |
| `global_struct_var` | グローバル構造体変数 | 13 | `global_var`, `struct_definition` |
| `var_scope_lookup` | 2段階の変数探索（ローカル → グローバル） | 13 | `global_var`, `alloc_local` |
| `local_shadows_global` | ローカル変数によるグローバル変数の隠蔽 | 13 | `var_scope_lookup` |
| `logical_not` | 単項 `!` | 13 | `bool_result_int01` |
| `logical_and_or` | 論理 `&&` `\|\|`（短絡しない） | 13 | `bool_result_int01` |
| `eval_order_no_shortcircuit` | 評価順序（両辺を評価する。例外 E3） | 13 | `logical_and_or` |
| `preprocess_provided` | スキャフォールド提供の前処理 `preprocess()` | 14 | `provided_lexer_parser` |
| `include_user_header` | `#include "自作ヘッダ"`（入れ子を含む） | 14 | `include_libh`, `preprocess_provided` |
| `define_object_macro` | `#define`（1段置換） | 14 | `preprocess_provided` |
| `multifile_compile` | 複数ソースファイルの同時コンパイル | 14 | `func_prototype`, `global_var` |
| `prototype_vs_definition` | プロトタイプと定義の分離 | 14 | `func_prototype`, `multifile_compile` |
| `include_cycle_error` | 循環取込み・マクロ多段参照のエラー | 14 | `include_user_header` |
...
- `codegen_lval` はコマ3 で `'Var'` だけの最小形として入れ、コマ7 で `'Deref'` へ
