...
| トラック | 条件 |
|---------|------|
| **標準** | Python 版 C サブセットコンパイラを完成させる（`fixed17` 全通が目安） |
...
上位トラックの参考達成条件は、`mycc_stage1` 生成成功、`fixed17` 全通、`preproc2`（`#define` 自前実装検証用）通過とする。
...
> `./mycc mycc.c -o mycc_stage1` に加え、`fixed17` 全通と前処理専用2本の通過を参考達成条件とする。
